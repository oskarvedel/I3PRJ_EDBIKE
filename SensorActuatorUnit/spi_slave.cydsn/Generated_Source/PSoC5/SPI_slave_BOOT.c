...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPI_slave) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: SPI_slave_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPI_slave_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPI_slave_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPI_slave_CyBtldrCommWrite
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    (void) SPI_slave_ReadTxStatus();

    /* Write TX data using blocking function */
    #if(0u != SPI_slave_MODE_USE_ZERO)
        SPI_slave_WriteTxDataZero(pData[buf_index]);
        buf_index++;
    #endif /* (SPI_slave_MODE_USE_ZERO == 1u) */

    /* Write TX data using blocking function */
    do
    {
        SPI_slave_WriteTxData(pData[buf_index]);
        buf_index++;
    }while(buf_index < size);

    while(0u != timeoutMs)
    {
        if(0u != (SPI_slave_ReadTxStatus() & SPI_slave_STS_SPI_DONE))
        {
            /* Return success code */
            *count = size;
...
/*******************************************************************************
* Function Name: SPI_slave_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    *  If atleast one byte is received within the timeout interval, wait for more data */
    for (iCntr = 0u; iCntr < (10u * (uint16) timeOut); iCntr++)
    {
        ...
        */
        if(0u != SPI_slave_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = SPI_slave_GetRxBufferSize();
                CyDelay(SPI_slave_BYTE2BYTE_TIME_OUT);
            }while(SPI_slave_GetRxBufferSize() > oldDataCount);
            status = CYRET_SUCCESS;
            break;
        }
    ...
    *count = 0u;
    ...
    while(SPI_slave_GetRxBufferSize() > 0u)
    {
        tempCount = SPI_slave_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = SPI_slave_ReadRxData();
                dataIndexCntr++;
            }
...
#endif /* if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPI_slave) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)) */
