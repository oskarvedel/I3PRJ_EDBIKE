...
/*******************************************************************************
* Function Name: Counter_clock1_Start
********************************************************************************
*
* Summary:
...
void Counter_clock1_Start(void) 
{
    ...
    Counter_clock1_CLKEN |= Counter_clock1_CLKEN_MASK;
	Counter_clock1_CLKSTBY |= Counter_clock1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Counter_clock1_Stop
********************************************************************************
*
* Summary:
...
void Counter_clock1_Stop(void) 
{
    ...
    Counter_clock1_CLKEN &= (uint8)(~Counter_clock1_CLKEN_MASK);
	Counter_clock1_CLKSTBY &= (uint8)(~Counter_clock1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Counter_clock1_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Counter_clock1_CLKEN & Counter_clock1_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Counter_clock1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Counter_clock1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Counter_clock1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Counter_clock1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Counter_clock1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Counter_clock1_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Counter_clock1_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Counter_clock1_GetSourceRegister();
    uint16 oldDivider = Counter_clock1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Counter_clock1_CLKEN & Counter_clock1_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Counter_clock1_DIV_PTR, clkDivider);
                Counter_clock1_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Counter_clock1_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Counter_clock1_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Counter_clock1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Counter_clock1_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Counter_clock1_DIV_PTR);
}


/*******************************************************************************
* Function Name: Counter_clock1_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Counter_clock1_MOD_SRC |= modeBitMask & (uint8)Counter_clock1_MODE_MASK;
}


/*******************************************************************************
* Function Name: Counter_clock1_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Counter_clock1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Counter_clock1_MODE_MASK));
}


/*******************************************************************************
* Function Name: Counter_clock1_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Counter_clock1_MOD_SRC & (uint8)(Counter_clock1_MODE_MASK);
}


/*******************************************************************************
* Function Name: Counter_clock1_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Counter_clock1_GetDividerRegister();
    uint8 oldSrc = Counter_clock1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Counter_clock1_MOD_SRC |= CYCLK_SSS;
        Counter_clock1_MOD_SRC =
            (Counter_clock1_MOD_SRC & (uint8)(~Counter_clock1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Counter_clock1_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Counter_clock1_MOD_SRC & Counter_clock1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Counter_clock1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Counter_clock1_PHASE = clkPhase & Counter_clock1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Counter_clock1_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Counter_clock1_PHASE & Counter_clock1_PHASE_MASK;
}
